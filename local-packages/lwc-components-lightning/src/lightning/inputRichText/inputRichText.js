...
import labelColorPicker from '@salesforce/label/LightningRichTextEditor.colorPicker';
...
import labelLinkInputTitle from '@salesforce/label/LightningRichTextEditor.linkInputTitle';
...
const DEFAULT_FONT_NAME_VALUE = 'default',
    ...
    SF_DEFAULT_FORMATS = ['table', 'image', 'link', 'header', 'blockquote'],
    ...
    ICON_VARIANT_BARE = 'bare',
    ICON_VARIANT_BARE_INV = 'bare-inverse';
...
const i18n = {
    ...
    linkInputTitle: labelLinkInputTitle,
...
function defaults(showImageUpload) {
    ...
    const insertContentLinkButton = {
        label: i18n.link,
        iconName: 'utility:link',
        format: 'link',
    };

    const insertContentImageButton = {
        label: i18n.image,
        iconName: 'utility:image',
        format: 'image',
    };

    const insertContentButtons = showImageUpload
        ? [insertContentLinkButton, insertContentImageButton]
        : [insertContentLinkButton];
...
const BUTTON_TOOLTIP_TYPE = TooltipType.Info;
 ...
 */
export default class LightningInputRichText extends LightningShadowBaseClass {
    ...
    */
    ...
    /**
     * Flag to check whether tooltips for all the buttons
     * are created or not.
     * Set to false again if any new button is added in the DOM
     * @type {boolean}
     * @default false
     */
    ...
    /**
     * Whether image upload button should be shown.
     * Since this feature relies on aura, the button is only shown when aura is available.
     * @type {boolean}
     * @default false
     */
     ...
     * The text is displayed in a tooltip above the rich text editor when you provide a label value.
     * To display the label next to the tooltip, use the label-visible attribute.
     * @type {string}
     */
    @api fieldLevelHelp;
    ...
    /**
     * Specifies whether users must enter content in the editor. If present, an asterisk is displayed before the label when label-visible is present.
     * @type {boolean}
     */
    @api required = false;
    ...
    set value(val) {
        ...
        if (typeof val === 'string' && this.internalValue !== val && isCSR) {
    ...
    focus() {
        ...
        if (!this.quill) {
            this.activateEditor();
            // Activate editor sets cursor at the end only when an event object is passed as an arugment to it.
            // Since there is no event object here, calling this method to set cursor at the end of text.
            this.focusAndSetCursorAtEnd();
        } else {
    ...
    linkTitle = ''; //Title to show in the input of link insertion panel
    fontMenus = {
        fontList: FONT_LIST,
    ...
    get computedCategories() {
        ...
        let categories = defaults(this.showImageUpload);
        if (customFormats) {
        ...
        categories.forEach((cat, index) => {
            ...
            cat.buttons.forEach((button) => {
                ...
                button.key = `${categoryKeyName}-${button.iconName}`;

                // add default primitive icon variant
                button.variant = ICON_VARIANT_BARE;
            });
        });
    ...
    /**
     * To get list of all rendered buttons
     */
    get buttonList() {
        return this.template.querySelectorAll(TOOLBAR_SELECTOR);
    }
    ...
    setupButtons() {
        if (isCSR) {
            const buttonList = Array.from(
                ...
                findAllFocusableNodes(this.template.querySelector('slot'))
            );
    ...
    addInitialClassesAndAttributesToEditor(qlEditor) {
        ...
        this.setAriaHiddenOnClipboardElement();
    }
    ...
    /**
     * Set aria-hidden to true on div.ql-clipboard to signal to
     * screen readers that this is not an visible area
     */
    setAriaHiddenOnClipboardElement() {
        const clipboardElement =
            this.template.querySelector('div.ql-clipboard');
        if (clipboardElement) {
    ...
    /**
     * set aria-required on the editor element based on if required is set to true or false
     */
    setAriaRequiredOnEditor() {
        if (this.quill) {
    ...
    /**
     * set aria-invalid on editor if the input is in invalid state
     */
    setAriaInvalidOnEditor() {
        if (this.quill) {
            const editor = this.quill.root;
            updateAriaInvalidOnElement(
                editor,
                computeAriaInvalid(!this._valid, this.value)
            );
        }
    }
    ...
    addTagMatchers() {
        ...
        this.quill.clipboard.addMatcher('font', (node, delta) => {
            ...
            return delta.compose(
                new lightningQuill.Quill.Delta().retain(
                    delta.length(),
                    nodeFormats
                )
            );
        ...
        this.quill.clipboard.addMatcher('s', (node, delta) => {
            return delta.compose(
                new lightningQuill.Quill.Delta().retain(delta.length(), {
                    strike: true,
                })
            );
        });
        ...
        this.quill.clipboard.addMatcher(
            ...
            (node, delta) => {
                ...
                const underline =
                    computedStyle.textDecoration.match(/underline/);
                const strike =
                    computedStyle.textDecoration.match(/line-through/);
                return delta.compose(
                    new lightningQuill.Quill.Delta().retain(delta.length(), {
        ...
        this.quill.clipboard.addMatcher('table', (node) => {
            ...
            const tableBlot = lightningQuill.Quill.import('formats/table');
            return new lightningQuill.Quill.Delta().insert({
    ...
    initializeQuill() {
        ...
        // Override S -> STRIKE
        const Strike = lightningQuill.Quill.import('formats/strike');
        Strike.tagName = 'STRIKE'; // Quill uses <s> by default
        lightningQuill.Quill.register(Strike, true);

        // Init Quill
        this.quill = new lightningQuill.Quill(container, quillConfig);
        ...
        if (this._isNativeShadow) {
            this.removeQuillNativeShadowListener = applyNativeShadowFixes(
                this.quill
            );
        }
        ...
        this.quill.on('text-change', () => {
            ...
            if (!this.quill.editor.isBlank()) {
                ...
                cleanedContent =
                    inputRichTextLibrary.cleanOutput(editorContents);
            }
        ...
        this.quill.scroll.domNode.addEventListener('dblclick', (clickEvt) => {
            ...
            if (linkNode) {
                ...
                let linkText =
                    linkNode.childNodes[0] && linkNode.childNodes[0].title
                        ? linkNode.childNodes[0].title
                        : linkNode.innerText;
                this.openLinkPanel(linkText, linkNode.getAttribute('href'));
            }
        });
        ...
        editor.addEventListener('blur', (event) => {
            ...
            if (targetClassName === 'ql-clipboard') return;
    ...
    connectedCallback() {
        ...
        getAura(() => {
            // Only show the image upload button when aura is available
            this.showImageUpload = true;

            // Call setupButtons() to set the tabIndex
            // for the newly added image upload button.
            // eslint-disable-next-line @lwc/lwc/no-async-operation
            requestAnimationFrame(() => {
                this.setupButtons();
            });

            // tooltipCreated is set to false as
            // tooltip needs to be created for newly added image upload button.
    ...
    disconnectedCallback() {
        ...
        for (let i = 0; i < buttonList.length; i++) {
            buttonList[i]._tooltip.disconnect();
        }
        ...
        // Removes events listener bound by quill native shadow fix
        if (
            this.removeQuillNativeShadowListener &&
            typeof this.removeQuillNativeShadowListener === 'function'
        ) {
            this.removeQuillNativeShadowListener();
        }
    }
    ...
    renderedCallback() {
        ...
        if (!this.tooltipCreated) {
            this.createButtonsTooltip();
        }
        this.setAriaRequiredOnEditor();
        this.setAriaInvalidOnEditor();
    }
    ...
    ******************************** */
    ...
    linkButtonClickHandler() {
        ...
        let selectedValue = '';
        if (selection.length !== 0) {
            selectedValue = quillApi.getText(selection.index, selection.length);
        }
        ...
        if (format.link) {
            ...
            this.openLinkPanel(selectedValue || '');
        }
    }
    ...
    /**
     * Update the link title to be saved as user types in create link panel input
     * @param {Event} event - Keyboard event to detect value of input
     */
    handleLinkTitleChange(event) {
        ...
        // During this case, detail doesn't exist and there is no change in value
        if (event.detail) {
            this.linkTitle = escapeHTML(event.detail.value);
        }
    }
     ...
     */
    openLinkPanel(title, value) {
        this.linkTitle = title || '';
        this.linkValue = value || '';
        const createLinkPanel =
            this.template.querySelector('[data-link-popup]');
    ...
    saveLink() {
        ...
        try {
            if (selection.length === 0) {
                quillApi.insertText(
                    selection.index,
                    this.linkTitle || this.linkValue,
                    { link: this.linkValue }
                );
            } else {
                this.addLinkToImagesAndText();
    ...
    /**
     * Sets title attribute of the image to the link's title.
     * Modifies the quill contents(text) to match to the link's title.
     */
    addLinkToImagesAndText() {
        const quillApi = this.quill;
        const selection = quillApi.getSelection(true);
        let contents = quillApi.getContents(selection.index, selection.range);
        let result = contents.ops.filter(
            (ele) => ele.insert && ele.insert.image
        );
        let quillImages;
        let querySelector;
        var oldTitle = '';
        this.linkTitle = escapeHTML(this.linkTitle || '');
        if (result) {
            for (let i = 0; i < result.length; i++) {
                querySelector =
                    result[i].attributes && result[i].attributes.alt
                        ? 'img[alt="' + result[i].attributes.alt + '"]'
                        : 'img[src="' + result[i].insert.image + '"]';
                quillImages =
                    quillApi.container.querySelectorAll(querySelector);
                for (let j = 0; j < quillImages.length; j++) {
                    quillImages[j].setAttribute('title', this.linkTitle);
                }
            }
        }
        oldTitle = this.quill.getText(selection.index, selection.length);

        if (oldTitle) {
            //this check is needed otherwise the images will be replaced with link's title
            oldTitle = oldTitle.replace(/(\r\n|\n|\r)/gm, '');
            const linkText = this.linkTitle ? this.linkTitle : this.linkValue;
            quillApi.deleteText(selection.index, oldTitle.length);
            quillApi.insertText(selection.index, linkText);
            quillApi.setSelection(selection.index, linkText.length);
        }
    }
    ...
    updateButtonPressedState(range) {
        ...
        buttonList.forEach((button) => {
            if (button) {
                ...
                // update button properties, depending on active state
                button.classList.toggle('slds-is-selected', isActive);
                button.variant = isActive
                    ? ICON_VARIANT_BARE_INV
                    : ICON_VARIANT_BARE;
    ...
    createTooltip(button) {
        button._tooltip = new Tooltip(button.title, {
            root: this,
            target: () => button,
            type: BUTTON_TOOLTIP_TYPE,
        });
        button._tooltip.initialize();
    }

    /**
     * List of buttons is populated from the DOM and
     * Tooltip is created for each button seperately
     */
    createButtonsTooltip() {
        const buttonList = this.buttonList;
        for (let i = 0; i < buttonList.length; i++) {
            this.createTooltip(buttonList[i]);
        }
