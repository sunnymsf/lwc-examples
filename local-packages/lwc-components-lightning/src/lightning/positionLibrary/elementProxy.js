...
export class ElementProxy {
    ...
    bake() {
        ...
        const adjustedAbsPosTop =
            absPos.top -
            (this.top < absPos.top
                ? pxToFloat(style.marginBottom)
                : -1 * pxToFloat(style.marginTop));
        const marginOffset =
            this.top < adjustedAbsPosTop
                ? pxToFloat(style.marginBottom)
                : pxToFloat(style.marginTop) * -1;

        let leftDif = Math.round(this.left - (absPos.left + scrollLeft));
        const topDif = Math.round(
            this.top - (absPos.top + marginOffset + scrollTop)
        );
