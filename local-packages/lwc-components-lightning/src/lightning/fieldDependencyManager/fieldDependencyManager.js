...
function updateDependentFields(fieldName, fieldValue, clearDependentValue) {
    ...
    for (let i = 0, len = dependentFieldNames.length; i < len; i++) {
        ...
        if (field) {
            ...
            const valueExistsInNewOptions = newOptions.some((option) => {
                return field.fieldElement.getFieldValue() === option.value;
            });

            if (clearDependentValue && !valueExistsInNewOptions) {
