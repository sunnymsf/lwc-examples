 ...
 */
export default class LightningTree extends LightningShadowBaseClass {
    /**
    ...
    constructor() {
        ...
        this.addEventListener('privateitemclick', this.handleClick.bind(this));
        this.addEventListener(
            'privateregisteritem',
    ...
    expandBranch(node) {
        if (!node.isLeaf && !node.isDisabled) {
            ...
            if (this._selectedItem?.key.startsWith(node.key)) {
