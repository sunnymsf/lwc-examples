import lazyQuill from './lazyQuill';
...
function ensureQuill() {
    // lazily initialize only when first requested
    if (!Quill) {
        Quill = lazyQuill();

        const BoldBlot = Quill.import('formats/bold');
        const ItalicBlot = Quill.import('formats/italic');
...
/**
 * These patches are required for quill to support native shadow, which no version currently does. See:
 * - https://github.com/quilljs/quill/issues/2021
 * - https://github.com/quilljs/quill/issues/2961
 **/
function applyNativeShadowFixes(quill) {
    // Check taken from the now defunct Google selection polyfill: https://github.com/GoogleChromeLabs/shadow-selection-polyfill/blob/master/shadow.js
    const hasShadowRootSelection = !!document
        .createElement('div')
        .attachShadow({ mode: 'open' }).getSelection;
    ...
    quill.selection.getNativeRange = function () {
        ...
        return nativeRange
            ? quill.selection.normalizeNative(nativeRange)
            : null;
    };
    ...
    if (isSafari) {
        quill.selection.setNativeRange = function (startNode, startOffset) {
            ...
            if (
                startNode != null &&
                (quill.selection.root.parentNode == null ||
...
function nestList(list, indentLevel, type) {
    ...
    while (list.length > 0) {
        ...
        if (lastNode && level > indentLevel) {
            ...
            indentLevel = computeIndentLevel(thisNode);
        ...
        aura.createComponent(
            'force:fileUpload',
            {
                ...
                onUpload: (serverResponse) => {
                    let attributes = { alt: file.name };
                    if (serverResponse.successful) {
                        if (
                            serverResponse.response &&
                            serverResponse.response.id
                        ) {
                            attributes['data-fileid'] =
                                serverResponse.response.id;
                        }
                        this.insertEmbed(
                            quillApi,
