...
export default class LightningModal extends LightningOverlay {
    ...
    connectedCallback() {
        // Safe for extended components to not call super
        // this is only to prevent `<lightning-modal>` element usage!
        if (this.constructor.name === 'LightningModal') {
            throw new Error(
                'Use of <lightning-modal> as an element is not supported. See developer documentation for extending the LightningModal class.'
            );
        }
    }
}
