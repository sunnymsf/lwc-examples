...
export default class LightningFormattedLookup extends LightningShadowBaseClass {
    ...
    get anchor() {
        if (isCSR && this._connected && this.isNavigable) {
     ...
     */
    updateLinkData() {
        if (isCSR && this._connected && this.state.recordId) {
            getLinkInfo(this, {
     ...
     */
    handleClick(event) {
        // disabled because function is not called on first paint
