...
The `content` attribute doesn't have a character limit. However, when this component is used for field-level help text on a Salesforce record, the character limit is 510. For more information, see [Define Field-Level Help](https://help.salesforce.com/s/articleView?id=sf.customhelp_defining_field_level_help.htm) in Salesforce Help.
